...
import type { AgentRole } from './runner.ts';
...
const AGENT_TO_TASK: Record<AgentRole, string> = {
  planner: "analysis",
  coder: "coding",
  reviewer: "review",
  debugger: "analysis",
...
async function withConcurrencyLimit(tasks: (() => Promise<void>)[], limit: number): Promise<void> {
  ...
  while (results.size < tasks.length) {
    ...
    await withConcurrencyLimit(
      ready.map((task) => async () => {
        ...
        const taskType = AGENT_TO_TASK[task.agent] as any;
        const recommendedModel = router.selectBestModel(taskType, { preferCheap: true });
        ...
        try {
          ...
          const timeoutPromise = new Promise<never>((_, reject) => {
            timeoutHandle = setTimeout(() => reject(new Error('Agent timeout')), task.timeout);
          });

          currentHistory = await Promise.race([
            runTurn(currentHistory, agentInput, turnCallbacks, task.agent, recommendedModel),
            timeoutPromise,
          ]);
          ...
          results.set(task.id, {
            taskId: task.id,
            agentName: task.agent,
            status: msg === 'Agent timeout' ? 'timeout' : 'error',
            output: msg,
            timestamp: Date.now(),
          });
        }
