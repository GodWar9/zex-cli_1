...
import { getAgentPrompt } from './agentPrompts.ts';
 ...
 */
import type { AgentRole } from './agentPrompts.ts';
export type { AgentRole };

/**
 * Executes one agent turn. If `agentRole` is set, injects role-specific
...
export async function runTurn(
  ...
  if (modelOverride) {
    config.provider = { ...config.provider, model: modelOverride };
  }
  ...
  // ── Inject role-specific agent prompt ──────────────────────────────────
  if (agentRole) {
    const rolePrompt = getAgentPrompt(agentRole);
    if (rolePrompt) {
      effectiveUserMsg = `[Role: ${agentRole}]\n${rolePrompt}\n\n${effectiveUserMsg}`;
    }
  }
  ...
  while (isTurnActive) {
    if (++turnCount > MAX_TURNS) {
      callbacks.onError(`Agent loop exceeded ${MAX_TURNS} turns — stopping to prevent runaway execution.`);
      break;
    }
    ...
    while (!streamDone) {
      ...
      try {
        ...
        const checkpointInfo = checkpointLog.length > 0
          ? `\n\n✓ Work saved before failure:\n${checkpointLog.map((c) => `  ${c}`).join('\n')}`
          : '';
        callbacks.onError((msg ?? 'Unknown error during stream') + checkpointInfo);
        return messages;
      }
    }
    ...
    if (toolCallsRequested.length > 0) {
      for (const tc of toolCallsRequested) {
        ...
        if (!toolDef) {
          ...
          if (approved) {
             let res: import('../tools/types.ts').ToolResult;
             try {
               res = await toolDef.execute(tc.args);
             } catch (execError: unknown) {
               const execMsg = execError instanceof Error ? execError.message : String(execError);
               res = { content: `Error executing tool ${tc.name}: ${execMsg}`, isError: true };
             }
  ...
  for (const c of chunks) {
    gc.releaseRef(c.id);
    gc.scheduleEviction(c.id, 5 * 60 * 1000);
  }
