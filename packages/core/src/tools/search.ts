...
import { validateWorkspaceBoundary } from '../utils/fs-guard.ts';
...
Returns matching file paths and line numbers with surrounding context.`,
  ...
  async execute({
    ...
    try {
      await validateWorkspaceBoundary(targetPath);
    } catch {
      return { content: `Error: Path ${targetPath} is outside the workspace boundary.`, isError: true };
    }

    // Build regex
    if (is_regex && query.length > 200) {
      return { content: 'Error: Regex pattern exceeds maximum length (200 chars).', isError: true };
    }
    let pattern: RegExp;
    try {
