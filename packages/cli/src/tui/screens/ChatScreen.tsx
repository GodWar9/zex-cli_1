...
export default function ChatScreen() {
  ...
  useEffect(() => {
    ...
    startup().catch((err) => {
      setMessages(prev => [...prev, {
        id: nextId(), role: 'system',
        content: `⚠ Startup error: ${err instanceof Error ? err.message : String(err)}`,
      }]);
    });
    startFileWatcher(process.cwd());
    gc.startBackgroundEviction();
    return () => {
      gc.stopBackgroundEviction();
    };
  }, [sessionId]);
    ...
    switch (result.type) {
      ...
      case 'config': {
        ...
        const lines = [
          ...
          '',
          `Edit ${join(homedir(), '.zex', 'config.json')} to change settings.`,
        ];
        setMessages(prev => [...prev, {
          id: systemId, role: 'system', content: lines.join('\n'),
        }]);
      ...
      case 'model': {
        ...
        setMessages(prev => [...prev, {
          id: systemId, role: 'system',
          content: `⚠ Model switching mid-session is not yet supported. Edit ${join(homedir(), '.zex', 'config.json')} and restart.\nWanted: ${result.modelName}`,
        }]);
        break;
      }
